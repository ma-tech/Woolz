...
#else
static char _WlzVerticies_c[] = "University of Edinburgh $Id$";
#endif
/*!
* \file         libWlz/WlzVerticies.c
* \author       Bill Hill
* \date         November 2000
* \version      $Id$
* \par
...
* \brief	Functions for extracting vertices from objects
* 		represented by vertices, eg polylines, boundlists
* 		and contours.
* \ingroup	WlzFeatures
*/
...
static WlzDVertex2  		*WlzDVerticesFromCMesh2D(
				  ...
				  int skip,
				  WlzErrorNum *dstErr);
static WlzDVertex3  		*WlzDVerticesFromCMesh2D5(
				  WlzCMesh2D5 *mesh,
				  int *dstCnt, 
				  int skip,
				  WlzErrorNum *dstErr);
...
* \ingroup	WlzFeatures
* \brief	Allocates a buffer which it fills with either 2D or
*		3D double precission vertices from the conforming
  ...
  if(mesh.v != NULL)
  {
	...
	*dstType = WLZ_VERTEX_D2;
	vData.d2 = WlzDVerticesFromCMesh2D(mesh.m2, dstCnt, skip, &errNum);
      ...
      case WLZ_CMESH_2D5:
	*dstType = WLZ_VERTEX_D3;
	vData.d3 = WlzDVerticesFromCMesh2D5(mesh.m2d5, dstCnt, skip, &errNum);
        break;
      case WLZ_CMESH_3D:
	*dstType = WLZ_VERTEX_D3;
...
*					may be NULL.
*/
static WlzDVertex3  *WlzDVerticesFromCMesh2D5(WlzCMesh2D5 *mesh, int *dstCnt, 
				        int skip, WlzErrorNum *dstErr)
{
  int		idN,
		idV,
  ...
  if(errNum == WLZ_ERR_NONE)
  {
    if(skip)
    {
      for(idN = idV = 0; idN < nCnt; ++idN)
      {
	cN = (WlzCMeshNod2D5 *)AlcVectorItemGet(vec, idN);
	if((cN->idx >= 0) && (idV < vCnt))
	{
	  *(vData.d3 + idV++) = cN->pos;
	}
