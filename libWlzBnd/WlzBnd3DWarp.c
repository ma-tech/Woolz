    ...
    *dstErr = errNum;
    ...
    WlzObject  *WlzBnd3DWarpObj(WlzObject *nullObjS, int arraySizeVec0, WlzDVertex3 *arrayVec0, int arraySizeVec1, WlzDVertex3 *arrayVec1, WlzErrorNum *dstErr)
    {
        FILE *inFile = NULL;   /* used to read Woolz object */ 
        int option,i;
        int nbelow                   = 2, 
  ...
  if((inFile = fopen("./smallVtk.wlz", "r")) == NULL )
  {
    printf("cannot open the input woolz file.\n");
    exit(1);
  }
   ...
   if(errNum == WLZ_ERR_NONE) {
        ...
        else
        {
                                           ...
                                           numOfElemAlonY, numOfElemAlonZ, &errNum) ) != NULL);
           {
              ...
              /* read the tie-points */
              if((inFile = fopen("./_gfTiePoint.tps", "r")) == NULL )
              {
                     printf("cannot open the input tie point data file.\n");
                     exit(1);
              }
  ...
  if(WARP)
  {
    wObjW = WlzMeshTransformObj_3D(wObjS,  wmt3D, interp, &errNum);
    /* output the woolz object */
    if((inFile = fopen("wraped.wlz", "w")) == NULL )
    {
       printf("cannot open output file.\n");
       exit(1);
    } 
