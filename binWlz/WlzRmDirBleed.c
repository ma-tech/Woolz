...
static WlzErrorNum		WlzRmDirBleed(
				  ...
				  int nrm,
				  WlzRasterDir dir);
static WlzErrorNum		WlzSetSectionInObject(
				  WlzObject *dObj,
				  WlzObject *sObj2,
				  ...
				  WlzThreeDViewStruct *vs)
{
  ...
  if((dObj == NULL) || (sObj2 == NULL))
  {
    errNum = WLZ_ERR_OBJECT_NULL;
  }
  else if((dObj->type != WLZ_3D_DOMAINOBJ) ||
          (sObj2->type != WLZ_2D_DOMAINOBJ))
  {
    errNum = WLZ_ERR_OBJECT_TYPE;
  }
  else if((dObj->domain.core == NULL) || (sObj2->domain.core == NULL))
  {
    errNum = WLZ_ERR_DOMAIN_NULL;
  }
  else if(vs == NULL)
  ...
  if(errNum == WLZ_ERR_NONE)
  {
    errNum = WlzInitGreyScan(sObj2, &iWSp, &gWSp );
  }
  if(errNum == WLZ_ERR_NONE)
  {
    while((errNum == WLZ_ERR_NONE) &&
          ((errNum = WlzNextGreyInterval(&iWSp )) == WLZ_ERR_NONE))
    {
      ...
      WlzDVertex3 q,
                  yp;
      gP = gWSp.u_grintptr;
      p.vtY = iWSp.linpos - WLZ_NINT(vs->minvals.vtY);
      yp.vtX = vs->yp_to_x[p.vtY];
      yp.vtY = vs->yp_to_y[p.vtY];
      yp.vtZ = vs->yp_to_z[p.vtY];
      switch(gWSp.pixeltype)
      {
	case WLZ_GREY_LONG:
	  for(i = iWSp.lftpos; i <= iWSp.rgtpos; ++i)
	  {
	    p.vtX = i - WLZ_NINT(vs->minvals.vtX);
	    q.vtX = yp.vtX + vs->xp_to_x[p.vtX];
	    q.vtY = yp.vtY + vs->xp_to_y[p.vtX];
	    q.vtZ = yp.vtZ + vs->xp_to_z[p.vtX];
	    WlzGreyValueGet(gVWSp, q.vtZ, q.vtY, q.vtX);
	    gVWSp->gPtr[0].lnp[0] = *(gP.lnp)++;
	  }
	  break;
	case WLZ_GREY_INT:
	  for(i = iWSp.lftpos; i <= iWSp.rgtpos; ++i)
